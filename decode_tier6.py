"""
Tier 6 decoder: 57 + new R, F, L, O candidates.
Test: 08=R, 20=F, 96=L, 99=O, 55=R
"""
import json
...
# Tier 6 candidates
tier6 = {
    '08': 'R',  # DER:10, RUNE:5, RUNEN:2 = 17 hits; left E:11,A:11,R:7; right E:14,A:10,N:7
    '20': 'F',  # FINDEN:11 hits; left D:11,E:7; right I:11,S:7,A:7
    '96': 'L',  # TEIL:8 hits; 96+CH=8 (MILCH/WELCH/SOLCH); left E:10,I:9
    '99': 'O',  # bigrams TO:14, SO:11, ON:19, OT:12 all fit O; TEIL:7 for L was misleading
    '55': 'R',  # DER:8 hits; left E:10,S:10,N:9; fills R deficit partially
}

all_codes = {**fixed, **tier6}
...
print("=" * 70)
print(f"TIER 6 DECODER: {len(all_codes)} fixed codes")
print("=" * 70)
...
# New words enabled by tier 6
print(f"\n{'='*70}")
print("NEW WORDS FROM TIER 6")
print("=" * 70)

new_words = ['FINDEN', 'RUNE', 'RUNEN', 'TEIL', 'OFT', 'FORT', 'DORT',
             'SOLCH', 'WELCH', 'MILCH', 'ODER', 'NOCH', 'VON',
             'DER', 'HIER', 'DORT', 'DOCH', 'HOCH',
             'ALLE', 'SOLL', 'WELT']
...
# Check unusual bigrams
print("\nPotentially unusual bigrams:")
unusual = ['RR', 'FF', 'LL', 'OO', 'OA', 'AO',
           'TL', 'FD', 'DF', 'LF', 'FL',
           'II', 'HH', 'DD', 'NN']
for bg in unusual:
...
print(f"\nExpected common bigrams:")
expected_common = ['EN', 'ER', 'CH', 'DE', 'EI', 'ND', 'TE', 'IN',
                   'IE', 'GE', 'ES', 'NE', 'SE', 'RE', 'HE', 'AN',
                   'UN', 'ST', 'BE', 'DI', 'OR', 'ON', 'OT', 'SO',
                   'NO', 'TO', 'RU', 'FI']
for bg in expected_common:
    ct = bigram_counts.get(bg, 0)
    if ct > 0:
        print(f"  {bg}: {ct}")


# Show URALTE STEINE context with new codes
print(f"\n{'='*70}")
print("URALTE STEINE CONTEXTS (with tier 6)")
print("=" * 70)

for i, bpairs in enumerate(book_pairs):
    target = ['61', '51', '35', '34', '78', '01', '92', '88', '95', '21', '60']
    for j in range(len(bpairs) - len(target)):
        if bpairs[j:j+len(target)] == target:
            start = max(0, j - 25)
            end = min(len(bpairs), j + len(target) + 25)
...
for c in unknown_codes:
    ...
    if freq >= 5:
        ...
        print(f"  {c} (freq={freq}): L={dict(left.most_common(4))} R={dict(right.most_common(4))}")


# Remaining frequency deficits
print(f"\nFrequency deficits still remaining:")
