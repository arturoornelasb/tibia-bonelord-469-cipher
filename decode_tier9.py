"""
Tier 9: 83 codes. KOENIG resolution + tier 8 corrections.
Removed: 22=D, 82=I (misassignments from KOENIG fragments)
Added: 22=K, 82=O, 73=N, 50=I, 84=G (from KOENIG pattern)
"""
import json
...
# 83 codes: tiers 1-7b (71) + tier 8 corrected (5) + KOENIG (5) + ZK (2)
all_codes = {
    # Tiers 1-4 (52 codes)
    ...
    # Tier 7b (3 codes)
    '44': 'U', '62': 'B', '68': 'R',
    # Tier 8a non-conflicting (5 codes)
    '23': 'S', '17': 'E', '29': 'E', '66': 'A', '49': 'E',
    # Tier 8b (2 codes)
    '38': 'K', '77': 'Z',
    # Tier 9: KOENIG resolution (5 codes — replaces 22=D, 82=I)
    '22': 'K',  # KOENIG position 1: DER KOENIG x10, fills K deficit
    '82': 'O',  # KOENIG position 2: fills O deficit (0.9%->1.2%)
    '73': 'N',  # KOENIG position 4
    '50': 'I',  # KOENIG position 5
    '84': 'G',  # KOENIG position 6: G from 2.4%->3.2% (expected 3.0%)
}
...
print("=" * 70)
print(f"TIER 9 DECODER: {len(all_codes)} codes (KOENIG resolved)")
print("=" * 70)
...
german_words = [
    ...
    'KOENIG', 'KOENIGE', 'KOENIGS', 'KOENIGLICH',
    'DERKOENIG', 'DERKOENIGIN',
    'KOENIGIN', 'KOENIGEN',
    'NIGEN', 'NIGEL',
    'GEHEIM', 'GEHEIMEN', 'GEHEIMNIS',
    'GEBOREN', 'GEMACHT', 'GESEHEN', 'GEWESEN',
    'GEFUNDEN', 'GELESEN', 'GEGEBEN', 'GESCHRIEBEN',
    'GESCHEHEN',
    'KEIN', 'KEINE', 'KEINEN',
    'KIND', 'KINDER',
    'KOERPER', 'KOMMEN', 'KONNTE',
    'IRGEND', 'JEMAND',
    'GEBEN', 'LEBEN', 'HABEN', 'OBEN', 'NEBEN',
]
...
for bg, ct in bigram_counts.most_common(30):
    print(f"  {bg}: {ct}")

print("\nUnusual (should be low):")
for bg in ['EE', 'II', 'HH', 'DD', 'NN', 'SS', 'AA', 'RR', 'UU',
           'KK', 'ZZ', 'GG', 'OO', 'DK', 'KD', 'KI', 'IK']:
