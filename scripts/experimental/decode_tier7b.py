"""
Tier 7b: 68 + additional strong assignments.
44=U (AUS:11), 62=B (AB=16), 68=R (ER=10,TR=13,RA=15,RCH=9)
"""
import json
...
# Tier 7b: strong evidence
tier7b = {
    '44': 'U',  # AUS:11 word hits; AU=11, EU=9 — strong German bigrams
    '62': 'B',  # AB=16 (84% of left) — extremely characteristic B pattern
    '68': 'R',  # TR=13, ER=10, RA=15, RCH=9 — perfect R profile (German's top bigrams)
}

all_codes = {**fixed, **tier7b}
...
print("=" * 70)
print(f"TIER 7b DECODER: {len(all_codes)} fixed codes")
print("=" * 70)
...
for idx, decoded, length in book_decoded[:12]:
...
# New words enabled
print(f"\n{'='*70}")
print("NEW WORDS ENABLED BY 7b")
print("=" * 70)

new_tests = ['AUS', 'ABER', 'AUCH', 'AUF', 'BEI', 'BIS',
             'BUCH', 'BERG', 'BALD', 'BAND', 'BRAUCH', 'BESCHRIEBEN',
             'HABEN', 'GEBEN', 'LEBEN', 'OBEN', 'NEBEN',
             'DURCH', 'BRAUCHEN', 'GEBRAUCHT', 'LAUBER']
...
# Check B-related bigrams
print("B-bigrams:")
for bg in ['AB', 'BE', 'BI', 'BU', 'BA', 'BR', 'BL', 'EB', 'UB', 'OB',
           'BB', 'BT', 'BS', 'BN', 'BD', 'BH', 'BM', 'BC']:
    ct = bigram_counts.get(bg, 0)
    if ct > 0:
        print(f"  {bg}: {ct}")

# Check R-related from new code 68
print("\nNew R(68) bigrams:")
for bg in ['RR', 'ER', 'RE', 'TR', 'RA', 'RC', 'RD', 'RN', 'RS', 'RI', 'RU', 'RO', 'RH']:
    ct = bigram_counts.get(bg, 0)
    if ct > 0:
        print(f"  {bg}: {ct}")
...
# Unusual bigrams
print("\nUnusual:")
for bg in ['EE', 'II', 'HH', 'DD', 'NN', 'RR', 'BB', 'UU', 'AA']:
    ct = bigram_counts.get(bg, 0)
    if ct > 0:
        print(f"  {bg}: {ct}")
