...
\begin{itemize}
\item E (16.4\% frequency) $\rightarrow$ 19 codes
\item N (9.8\%) $\rightarrow$ 9 codes
\item S (7.3\%) $\rightarrow$ 8 codes
\item R (7.1\%) $\rightarrow$ 7 codes
\item \ldots down to Z (1.1\%) $\rightarrow$ 1 code
\end{itemize}
...
\begin{itemize}
\item OEDE (4 letters, "wasteland") --- requires 2 I$\rightarrow$E swaps
...
\textbf{Input:} A garbled block $G$ of length $n$, a dictionary $D$ of known words, and a set of permitted letter swaps $S$ (e.g., \{I$\leftrightarrow$E, I$\leftrightarrow$L\}).
...
\begin{itemize}
\item \textbf{I$\leftrightarrow$E swap:} The 6 I-codes and 19 E-codes create boundary confusion where a code pair decoded as \texttt{...I} might actually end in \texttt{E} if the pair boundary is shifted
\item \textbf{I$\leftrightarrow$L swap:} The 2 L-codes (34, 96) are numerically adjacent to I-codes, causing occasional misassignment
\end{itemize}
...
Total: \textasciitilde12,000 entries (after filtering to the cipher's 20-letter alphabet).
...
\begin{itemize}
\item \texttt{WINDUNRUH} $\rightarrow$ \texttt{WINDNURH} (breaks the compound "Windunruh" = wind-unrest)
\item \texttt{SCHAUNRUIN} $\rightarrow$ \texttt{SCHAUNRUIN} (breaks "schaun ruin" = behold ruin)
\end{itemize}
...
This resolved all 8 occurrences of the UNR$\rightarrow$NUR anagram without affecting the 4 occurrences of WINDUNRUH.
...
The I$\rightarrow$E swap dominates (87\% of all swaps), consistent with E being the most frequent letter (19 codes) creating the most boundary ambiguity.
...
\item \textbf{Swap model specificity:} The I$\leftrightarrow$E and I$\leftrightarrow$L swaps are specific to this cipher's code distribution --- other ciphers would require different swap models
...
\vfill
\begin{center}
\textit{This work is licensed under the MIT License. See LICENSE in the repository root.}
\end{center}
