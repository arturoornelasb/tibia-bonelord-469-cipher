...
\begin{itemize}
\item 3$\rightarrow$3: 1 occurrence (0.01\%)
\item 3$\rightarrow$2: 2 occurrences (0.02\%)
\item 0$\rightarrow$7: 1 occurrence (0.01\%)
\end{itemize}
...
\item Largest overlap: Book 36 $\rightarrow$ Book 11 = 279 digits (97.6\% of Book 36)
...
6--7 & Context analysis, NPC cribs & T(6), I(6), A(6), O(5) & Contextual \\
8--10 & Word completion, compound analysis & H(4), M(4), U(4), W(3), G(3) & Semantic \\
11--14 & Bigram validation, gap analysis & L(2), K(2), C, F, B, Z & Residual \\
\bottomrule
\end{tabular}}
\end{table}

Each tier was validated against German bigram/trigram frequencies before proceeding. By Tier 14, \textbf{98 of 100 possible codes} were assigned to 20 German letters (codes 07 and 32 never appear in any book).
...
Brute-force testing of all unconfirmed codes against all 20 possible letter assignments (Session 25) found no improvements exceeding +6 characters. The mapping is confirmed stable.
...
Example: \texttt{LABGZERAS} $\rightarrow$ SALZBERG (exact anagram + 1 extra letter A)
...
Example: \texttt{SERTI} $\rightarrow$ STIER (bull), \texttt{ESR} $\rightarrow$ SER (very), \texttt{NEDE} $\rightarrow$ ENDE (end)

\textbf{3. Bag-of-Letters Word Partition (Sessions 28--30)}

The key innovation: decomposing a garbled block's letter bag into the \textbf{optimal combination} of known German words, allowing I$\leftrightarrow$E and I$\leftrightarrow$L letter swaps (confirmed cipher obfuscation patterns).

Example: \texttt{DNRHAUNIIOD} (11 letters) $\rightarrow$ OEDE (4) + NUR (3) + HAND (4) = 11/11 coverage, with 2 I$\rightarrow$E swaps.

\subsection{Letter Swap Patterns}

Two systematic letter swaps were identified:
\begin{itemize}
\item \textbf{I$\leftrightarrow$E}: The cipher's 6 I-codes and 19 E-codes create boundary ambiguity
\item \textbf{I$\leftrightarrow$L}: Two L-codes (34, 96) are sometimes confused with I-codes
\end{itemize}
...
\item \textbf{Replication}: Any researcher can reproduce results using \texttt{mapping\allowbreak\_v7.json} and \texttt{narrative\allowbreak\_v3\allowbreak\_clean.py}
\end{enumerate}
...
\begin{enumerate}
\item \textbf{Homophonic substitution} (98 codes for 20 letters) defeats simple frequency analysis
...
\begin{itemize}
\item Complete German $\rightarrow$ English scholarly translation of the full text
...
\textbf{Note:} Two important tasks remain before the solution can be considered fully validated: (1)~NPC dialogues containing 469 sequences (Evil Eye, Elder Bonelord, Avar Tar) have not yet been decoded using the v7 mapping and may reveal additional context or confirm the cipher's scope; and (2)~all 70 book transcriptions must be verified against the current in-game text to rule out transcription errors that could affect downstream decoding.
