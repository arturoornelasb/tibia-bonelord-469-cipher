"""
Full decode with URALTE-derived assignments + tier 1/2.
Fix all high-confidence codes, optimize remaining ~60 codes.
"""
import json
import random
...
# ALL fixed assignments: confirmed + tier1 + tier2 + tier3 (URALTE)
fixed = {
    # Confirmed (16)
    ...
    '57': 'H', '04': 'M', '12': 'S', '58': 'N',
    # Tier 3 - URALTE derived (8)
    '78': 'T', '51': 'R', '35': 'A', '34': 'L',
    '01': 'E', '59': 'S', '41': 'E', '30': 'E',
    # High-confidence from crack_unknowns: 94=H (CH pattern)
    '94': 'H',
}
...
# Check current distribution with fixed codes
...
print(f"\nCurrent fixed code distribution:")
for li in range(26):
    l = letters[li]
    if letter_fixed_count[l] > 0 or german_freq.get(l, 0) > 0.01:
        exp = german_freq.get(l, 0.001)
        obs = letter_fixed_freq[li]
        need = max(0, exp - obs)
        print(f"  {l}: {letter_fixed_count[l]} codes, {obs*100:.1f}% obs (exp {exp*100:.1f}%, need {need*100:.1f}% more)")
...
# Sort free codes by frequency (high freq codes first)
...
# German words + bigrams for scoring
...
def freq_score(marr):
    ...
    for li in range(26):
        ...
        score -= weight * (obs - exp) ** 2 * 2000  # Strong penalty
    return score
...
# HILL CLIMBING
print(f"\n{'='*70}")
print("HILL CLIMBING (single code changes)")
print(f"{'='*70}")
...
for idx, decoded in book_decoded[:8]:
...
print(f"\nSaved mapping to best_mapping.json")
