#!/usr/bin/env python3
"""Session 28 Round 2: Re-run swap attack on 81.0% baseline to find more gains."""
import json, os
...
print(f"SESSION 28 R2 BASELINE: {base_cov}/{total} = {base_cov/total*100:.1f}%")

tokens = dp_segment_full(base_tr, base_known)

# Show remaining garbled inventory
...
# Show all books with full decoded text
print("\n" + "="*70)
print("ALL BOOKS (Session 28 R2)")
print("="*70)
...
for bidx, bpairs in enumerate(book_pairs_list):
    ...
    book_cov = dp_score(book_text, base_known)
    ...
    print(f"\n  Book {bidx:2d} ({pct:5.1f}%) {marker}")
    print(f"    {display}")

# Top remaining garbled by impact
print("\n" + "="*70)
print("TOP REMAINING GARBLED BLOCKS")
print("="*70)
scored = [(len(v)*c, v, c) for v,c in garbled_blocks.items()]
scored.sort(key=lambda x: -x[0])
for impact, block, count in scored[:20]:
    ctxs = garbled_contexts[block]
